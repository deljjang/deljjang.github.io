...
function bibleDataLoad() {
	...
	if($("#edSearch") == null || $("#edSearch").val() == undefined || $("#edSearch").val() == "") {
		searchWordYN = false;
  		rootRef.child("bible").once('value', function(data){ //once value, child_removed, child_changed, child_moved
  		    ...
  		    //console.log('memoQuery.memo.data=' , data.val());
  		  	...
  		  	for(var tmp in data.val()) {
  		  		...
  		  		//console.log('bibleDataLoad.tmp=' , tmp , " " + dataVal);
  		  		
  		  		//rootRef.child("bible/"+ dataVal.cd1 + "/" + dataVal.cd2).set(dataVal);
  		  		/*
  		  		if(tmp != "1" && tmp != "2") {
  	  		  		rootRef.child("bible/"+ tmp).remove();
  		  		}
  		  		*/
  		  	}
  		  	/* 정렬시 사용
...
/*	
	rootRef.child("bible_txt").orderByChild('cd1').equalTo("2").once('value', function(data){ //once value, child_removed, child_changed, child_moved
	    ...
	    //console.log('memoQuery.memo.data=' , data.val());
	  	...
	  	for(var tmp in data.val()) {
	  		...
	  		console.log('bibleDataLoad.tmp=' , tmp , ", 2=" + dataVal.cd2 + "/" + dataVal.cd3 + "/" + dataVal.cd4);
	  		
	  		rootRef.child("bible_2/"+ dataVal.cd2+"/"+ dataVal.cd3+"/"+ dataVal.cd4).set(dataVal);
	  	}
	});  		
*/
...
function treeDataAll(jsonAll) {
	for(var tmp in jsonAll) {
		...
		console.log("treeDataAll.tmp=" + tmp + ", key=" + json.key);
	...
	var rootRef = firebase.database().ref();
	var memoQuery = rootRef.child("bible_song");
	memoQuery.once('value', function(_data){
		var node = {
			text: "찬송",
			href: 'javascript:showDetail(\"3_\",\"\",\"\");'  ,
			key: "3_",
			cd1: "3",
			cd2: "",
			cd3: "",
		...
		for(var _key in _data.val()) {
			var json = _data.val()[_key];
			//console.log("key=" + _key + "/" + json.cd2 + "/"+json.nm);
			
			var cnode = {
					text: json.nm,
					href: 'javascript:showDetail(\"3_\",\""+json.cd2+"\",\"\");'  ,
					key: "3_",
					cd1: "3",
					cd2: json.cd2,
					cd3: "",
 ...
 * @param node
 */
function treeSubData(sub, node) {

	var jsonSort = [];
  	for(var tmp in sub) {
  		var json = sub[tmp];
  		//console.log('treeSubData.tmp=' , tmp , ", cd1=" + json.cd1 + ", cd2=" + json.cd2 + " " + json.nm + " " + json.nm2);
		...
		var nodes = {
			text: title,
			href: 'javascript:showDetail(\""+json.cd1+"\",\""+json.cd2+"\",\"\");'  ,
			key: json.cd1+"_"+json.cd2,
			cd1: json.cd1,
			cd2: json.cd2,
			cd3: '',
			nm2: json.nm2,
		...
		if(json.cd3 != null) {
			nodes.tags = [json.cd3+""];
			nodes.nodes = [];
	      	treeSubCd3(nodes, nodes.nodes, json.cd3);
		}
...
//장 생성
function treeSubCd3(json, node, cd3) {
	for (var i = 1; i <= cd3; i++) {
		var nodes = {
			text: json.nm2+i+"장",
			href: 'javascript:showDetail(\""+json.cd1+"\",\""+json.cd2+"\",\""+i+"\");'  ,
			key: json.cd1+"_"+json.cd2+"_"+i,
			cd1: json.cd1,
			cd2: json.cd2,
			cd3: i+'',
			nm2: json.nm2,
		...
		node.push(nodes);
		//console.log('treeSubCd3=' + json.nm2 + i);
	}
}

//index에 맞는 노드를 찾는다.
function findDefaultData(cd1, cd2, cd3, data) {
	for(var tmp in data) {
		var node = data[tmp];
		if(cd1 == node.cd1 && cd2 == node.cd2 && cd3 == node.cd3) {
			//console.log('findDefaultData.key=' + node.key + ', cd1=' + node.cd1 + ", cd2=" + node.cd2 + ", " + node.text);
			return node;
		}
		if(node.nodes != null) {
			var tmpNode = findDefaultData(cd1, cd2, cd3, node.nodes);
			if(tmpNode != null) {
 ...
 * tree클릭시 처리 
 */
function showDetail(index, cd2, cd3) {
	...
	console.log('showDetail.defaultDataNode=' + index + ", cd2=" + cd2);
	
	var defaultDataNode = findDefaultData(index, cd2, cd3, defaultData);

	if(index == "3" && cd2 != "") {
		console.log('showDetail.defaultDataNode=' + defaultDataNode.text);
		$("#btnBodyDiv").html("<div class='col-12' style='word-break:break-all;'>" 
		  		+ "<a href='#' onclick='javascript:selectNodeByKey(\"" + defaultDataNode.key + "\");'><font color='red'>" + defaultDataNode.text + "</font></a>"
		  		+ "</div>");
		
		var html = "<img src='/img/bible_song/"+cd2+".gif'>";
		html = "<div id='bodyDiv' class='col-12' style='word-break:break-all;'>"+html+"</div>";
		$("#treeBodyDiv").html(html);
		...
		var memoQuery = rootRef.child("bible_song_txt/"+cd2);//.orderByChild("cd2").equalTo(cd2);
		console.log('showDetail.3.memoQuery=' + memoQuery);
		memoQuery.once('value', function(_data){
			if(_data.val() != null) {
				//console.log('showDetail.3.txt=' + _data.val().txt);
				$("#treeBodyDiv").append("<br>"+_data.val().txt);
			}
		});
	...
	//console.log('showDetail.defaultDataNode=' + defaultDataNode);
	console.log('showDetail.defaultDataNode.cd=' + index + "/" + defaultDataNode.cd2 + "/"+defaultDataNode.cd3 + ", " + defaultDataNode.text);
	...
	if(defaultDataNode.memo == null || defaultDataNode.memo == undefined || defaultDataNode.memo == "") {
		...
		var memoQuery = rootRef.child("bible_"+index+"/"+cd2+"/"+cd3);//.orderByChild("cd2").equalTo(cd2);
		console.log('showDetail.memoQuery=' + memoQuery);
		
		memoQuery.once('value', function(_data){
			$("#treeBodyDiv").html("");
			$("#btnBodyDiv").html("");
			
			$("#btnBodyDiv").append("<div class='col-12' style='word-break:break-all;'>" 
			  		+ "<a href='#' onclick='javascript:selectNodeByKey(\"" + defaultDataNode.key + "\");'><font color='red'>" + defaultDataNode.text + "</font></a>"
			  		+ "</div>");
			//var cnt=0;
	  		if(_data.val() != null) {
	  			...
	  			for(var _key in _data.val()) {
		  			...
		  			console.log("showDetail.defaultDataNode._key=" + json + ", cd3=" + defaultDataNode.nm2 + json.cd3 + ":" + json.cd4 + ", " + json.txt);
		  			html += "<tr>";
		  			html += "<td width='60px'><font color='blue'>"+defaultDataNode.nm2 + json.cd3 + ":" + json.cd4 + "</font></td>";
		  			html += "<td>"+json.txt+ "</td>";
		  			
		  			//$("#treeBodyDiv").append("<div id='bodyDiv' class='col-12' style='word-break:break-all;'><font color='blue'>" + defaultDataNode.nm2 + json.cd3 + ":" + json.cd4 + "</font>&nbsp;&nbsp;" + json.txt + "</div>");				
		  			html += "</tr>";
	  				//cnt++;
	  			}
	  			html = "<div id='bodyDiv' class='col-12' style='word-break:break-all;'><table><tbody>"+html+"</tbody></table></div>";
	  			$("#treeBodyDiv").html(html);				
	  		...
	  		//rootRef.child('bible/'+index+'/'+cd2+'/cd3').set(cnt);
		});		
	} else {
